      ...
      >
        {navItems.map((navItem, idx) => (
          <Link
            ...
            href={navItem.link}
            className={cn(
              "relative dark:text-neutral-50 items-center flex space-x-1 text-neutral-400 hover:text-white transition-colors duration-300"
            )}
          >
            <span className="block">{navItem.icon}</span>
            {/* 👇 الاسم يظهر فقط من شاشة sm وأكبر */}
            <span className="hidden sm:block text-sm text-white hover:text-neutral-400 duration-300 !cursor-pointer">
              {navItem.name}
            </span>
          </Link>
        ))}
